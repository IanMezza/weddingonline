/* global weddingonlineIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'weddingonline-supports-dark-theme' ) ) {
	weddingonlineDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function weddingonlineDarkModeEditorInit( attempt ) {
	...
	if ( weddingonlineIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						weddingonlineDarkModeEditorInit( attempt + 1 );
					},
